...
const generatePlaceholderTexture = (index: number) => {
...
// Helper: Load image texture with fallback to placeholder
...
const InstaxFrame: React.FC<{
  ...
  const [currentTexture, setCurrentTexture] = useState<THREE.Texture>(texture);
  ...
  useFrame((state, delta) => {
    ...
    if (isGalaxy && !isActive) {
      ...
      meshRef.current.rotation.z = damp(
...
const InstaxGallery: React.FC = () => {
  ...
  useFrame((state) => {
    ...
    if (gesture === "THREE_FINGERS" && viewMode) {
      if (now - switchCooldown.current > 0.8) {
        // 0.8s wait between switches
  ...
  const frames = useMemo(() => {
    ...
    return Array.from({ length: PHOTO_COUNT }).map((_, i) => {
      // Start with placeholder - actual image will load in useEffect
      const texture = generatePlaceholderTexture(i);
